                ...
                @forelse($incidents as $incident)
                    <div class="feed-card">
                        ...
                        <div class="flex items-center mb-3">
                            ...
                            <div class="ml-3">
                                <p class="font-semibold text-gray-800">{{ $incident->user->name ?? 'Utilisateur inconnu' }}</p>
                                <p class="text-xs text-gray-500">{{ $incident->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                        ...
                        <div class="mb-3 flex items-center justify-between">
                            <span class="font-bold text-gray-700 text-lg">
                                {{ $incident->type->name ?? 'Type inconnu' }}
                            </span>

                            <!-- Status -->
                            <span class="status-badge 
                                @if($incident->status=='recu') bg-blue-100 text-blue-600
                                @elseif($incident->status=='en_cours') bg-yellow-100 text-yellow-700
                        ...
                        <!-- MEDIA (IMAGES) -->
                        @if($incident->media && is_array($incident->media) && count($incident->media) > 0)
                            <div class="grid grid-cols-2 gap-3 mb-4">
                                @foreach($incident->media as $img)
                                    <img src="{{ asset('storage/'.$img) }}" class="rounded-lg w-full h-48 object-cover shadow">
                                @endforeach
                            </div>
                        @endif
                        ...
                        <div class="flex items-center justify-between mt-4 border-t pt-3">
                            <!-- VOTE -->
                            <form action="{{ route('citizen.incidents.vote', $incident->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="text-gray-600 hover:text-pink-500 font-medium flex items-center transition-colors">
                                    ...
                                    <span>Confirmer ({{ $incident->votes_count ?? 0 }})</span>
                                </button>
                            </form>

                            <!-- COMMENTS -->
                            <a href="{{ route('citizen.incidents.show', $incident->id) }}"
